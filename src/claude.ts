...
class ClaudeSession extends EventEmitter {
  ...
  private sendMessageInternal(
    ...
    if (!this.proc || !this.proc.stdin) {
      resolve({
        success: false,
        response: "",
        error: "Claude session not started",
      });
      return;
    }
  ...
  async sendMessage(
    ...
    return new Promise((resolve) => {
      ...
      const wrappedResolve = (response: ClaudeResponse) => {
        if (resolved) return;
        resolved = true;
        if (timeoutId) {
...
export async function runClaude(
  ...
  if (breakerState === "open") {
    return {
      success: false,
      response: "",
      error: "Claude CLI circuit breaker is open due to repeated failures. Try again later.",
    };
  }
